...
export const EmployeeAssess: React.FC<{ user: any, setUsers: any }> = ({ user, setUsers }) => {
    ...
    const [scores, setScores] = useState<any>({ 0: 3, 1: 4, 4: 2 });
    ...
    const sections = {
        CC: [
            { cd: "CC-001", n: "การบริการที่ดี", beh: ["ตอบสนองความต้องการผู้รับบริการได้ทันท่วงที", "ให้ข้อมูลที่ถูกต้องและครบถ้วนแก่ผู้รับบริการ", "แสดงความเป็นมิตร ยิ้มแย้ม ให้บริการด้วยใจ"] },
            { cd: "CC-003", n: "การทำงานเป็นทีม", beh: ["แบ่งปันข้อมูลและทรัพยากรกับทีมอย่างเต็มที่", "รับฟังความคิดเห็นผู้อื่นด้วยใจเปิดกว้าง", "ช่วยเหลือเพื่อนร่วมงานเมื่อมีปัญหา"] },
            { cd: "CC-004", n: "จริยธรรมและความซื่อสัตย์", beh: ["ปฏิบัติงานด้วยความซื่อสัตย์สุจริต", "รักษาความลับขององค์กรได้เป็นอย่างดี", "ยึดมั่นในหลักจริยธรรมวิชาชีพ"] }
        ],
        MC: [],
        FC: [
            { cd: "FC2-061", n: "การใช้เทคโนโลยีดิจิทัล", beh: ["ใช้โปรแกรมสำนักงานได้คล่องแคล่ว", "ใช้เครื่องมือดิจิทัลในการวิเคราะห์ข้อมูล", "ประยุกต์ใช้ AI เพื่อเพิ่มประสิทธิภาพงาน"] },
            { cd: "FC2-062", n: "การวิเคราะห์ข้อมูล", beh: ["รวบรวมและวิเคราะห์ข้อมูลได้อย่างเป็นระบบ", "นำเสนอข้อมูลในรูปแบบที่เข้าใจง่าย", "ใช้ข้อมูลในการตัดสินใจและแก้ปัญหา"] }
        ]
    };
                                        ...
                                        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "8px", marginBottom: "14px" }}>
                                            {[1, 2, 3, 4, 5].map(k => (
                                                <div key={k} onClick={() => setScores({ ...scores, [curIdx]: k })} style={{ border: `2px solid ${score === k ? "var(--teal)" : "var(--border)"}`, borderRadius: "10px", padding: "14px 8px", cursor: "pointer", background: score === k ? "var(--teal-lt)" : "#fff", transition: ".15s", textAlign: "center" }}>
                                                    ...
                                                    <div style={{ fontSize: "10px", color: "var(--text3)", marginTop: "4px" }}>{["ต่ำมาก", "ต่ำ", "พอใช้", "ดี", "ดีมาก"][k - 1]}</div>
                                                </div>
                                            ))}
                                        </div>
